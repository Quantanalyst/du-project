...
############# finding the HR number of devices #############
...
device = 'B'
number = 101
## this piece is responsible to find the HR list of the desired device
hr_list = []
for val in np.arange(0,len(devicelist)):
    if str(devicelist[val]) == 'nan':
        pass
    elif devicelist[val][0] == str(device) and devicelist[val][1] == str(number):    # if it finds a match, it will append the hr_list
